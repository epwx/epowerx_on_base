...
it('places paired wash SELL immediately in SELF_TRADE_MODE=on even when confirmation feed is empty', async () => {
  const mockExchange = {
    getBalances: jest.fn().mockResolvedValue([
      { asset: 'USDT', free: 500, locked: 0, total: 500 },
      { asset: 'EPWX', free: 1000000000000, locked: 0, total: 1000000000000 }
...
describe('Confirmed wash fill polling', () => {
  ...
  it('credits wash carry when pending snapshot reports delayed BUY fills', async () => {
    const mockExchange = {
      getBalances: jest.fn(),
      getTicker: jest.fn(),
      getOpenOrders: jest.fn().mockResolvedValue([]),
      getOrder: jest.fn().mockResolvedValue({
        orderId: 'wash-buy-late-1',
        symbol: 'EPWXUSDT',
        side: 'BUY',
        type: 'LIMIT',
        price: 1,
    ...
    (strategy as any).activeOrders.set('wash-buy-late-1', {
      orderId: 'wash-buy-late-1',
      symbol: 'EPWXUSDT',
      side: 'BUY',
      type: 'LIMIT',
    ...
    (strategy as any).washSubmittedOrderIds.add('wash-buy-late-1');

    await (strategy as any).pollOrderFills('wash-buy-late-1', 'BUY', true);

    expect((strategy as any).washConfirmedBuyCarryAmount).toBe(7);
